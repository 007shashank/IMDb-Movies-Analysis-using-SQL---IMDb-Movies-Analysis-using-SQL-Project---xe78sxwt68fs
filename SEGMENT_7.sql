...
left join imdb_ratings r on r.movie_id=m.id
...
row_number() over (order by avg(avg_rating) desc) as ranks from imdb_movies m
left join imdb_ratings r on r.movie_id=m.id
...
from imdb_movies m
left join imdb_ratings r on r.movie_id=m.id
left join imdb_role_mapping d on d.movie_id=m.id
left join imdb_names n on n.id=d.name_id
where d.name_id is not null
and category='actor'
group by name_id,n.name) a where ranks<=10;
...
row_number() over (order by sum(worlwide_gross_income) desc) as ranks
from imdb_movies group by production_company) a where ranks<=10;
...
row_number() over (order by worlwide_gross_income desc) as ranks from imdb_movies) a where ranks<=100) a;
...
row_number() over (order by avg(avg_rating) desc) as ranks from imdb_movies
left join imdb_ratings on movie_id=id
...
row_number() over (partition by genre order by avg(avg_rating) desc) as ranks from imdb_movies
left join imdb_ratings r on r.movie_id=id
left join imdb_genre g on g.movie_id=id
...
row_number() over (order by sum(worlwide_gross_income) desc) as ranks
from imdb_movies where country not like '%,%'
...
row_number() over (partition by genre order by avg(avg_rating) desc) as ranks from imdb_movies m
